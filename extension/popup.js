...
function showTask(task) {
  ...
  $("copyOutput").textContent = task?.keyword ? "复制 Keyword + Hashtag" : "复制 Hashtag";
...
function renderTaskList() {
  ...
  for (const task of sorted) {
    ...
    title.textContent = task.keyword || task.hashtags?.[0] || "未识别任务";
    ...
    copy.addEventListener("click", async () => {
      ...
      await navigator.clipboard.writeText([validation.keyword, ...validation.hashtags].filter(Boolean).join("\n"));
      setToast("已复制");
    });
